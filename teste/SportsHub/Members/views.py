...
from django.shortcuts import render
from .models import FitnessTrainer, TrainerUserConnection, TrainingPlanAssignment,EquipmentReservation
...
def fitness_trainer_dashboard(request):
    ...
    recent_reservations = EquipmentReservation.objects.filter(trainer=logged_in_trainer).order_by('-date')[:5]
    ...
    assigned_plans = WeeklyFitnessPlan.objects.filter(
